...
static bool test_tt_bucket_collisions_fit() {
    ...
    for (const auto& entry : entries) {
        ...
        ASSERT(probed, "tt_bucket_collisions_fit", "Missing colliding entry in bucket");

        ASSERT(entry_matches(*probed, entry.hash, entry.best_move, entry.depth, entry.score, entry.node, 4), "tt_bucket_collisions_fit", "Colliding entry fields mismatch");
    }
...
static bool test_tt_replace_same_hash() {
    ...
    ASSERT_EQ(matches, 1, "tt_replace_same_hash", "Bucket should contain exactly one copy of hash");
...
static bool test_tt_bucket_replacement_by_depth() {
    ...
    ASSERT(!g_transposition_table.get_entry(should_be_replaced), "tt_bucket_replacement_by_depth", "Lowest depth entry should be replaced");

    TTEntry* replacement = g_transposition_table.get_entry(colliding_hash(15));
    ASSERT(replacement, "tt_bucket_replacement_by_depth", "Replacement entry missing");

    ASSERT(entry_matches(*replacement, colliding_hash(15), Move(E2, E3, MoveType::Quiet, MoveFlag::Normal), 6, 99, TTNode::FailHigh, 0), "tt_bucket_replacement_by_depth", "Replacement entry fields mismatch");
...
static bool test_tt_bucket_replacement_by_age() {
    ...
    ASSERT(!g_transposition_table.get_entry(hashes[0]), "tt_bucket_replacement_by_age", "Oldest entry should be replaced");

    for (size_t i = 1; i < hashes.size(); i++) {
        ASSERT(g_transposition_table.get_entry(hashes[i]), "tt_bucket_replacement_by_age", "Newer entry was replaced unexpectedly");
    }

    TTEntry* replacement = g_transposition_table.get_entry(replacement_hash);
    ASSERT(replacement, "tt_bucket_replacement_by_age", "Replacement entry missing");

    ASSERT_EQ(replacement->age, 8, "tt_bucket_replacement_by_age", "Replacement entry has wrong stamped age");
...
static bool test_pawn_table_store_probe() {
    ...
    ASSERT(g_pawn_table.is_valid_entry(entry.hash, probed), "g_pawn_table_store_probe", "Entry not valid after store");
...
static bool test_pawn_table_invalid_probe() {
    ...
    ASSERT(!g_pawn_table.is_valid_entry(hash, empty), "g_pawn_table_invalid_probe", "Empty entry should be invalid");
    ...
    ASSERT(!g_pawn_table.is_valid_entry(wrong_hash, wrong_probed), "g_pawn_table_invalid_probe", "Mismatched hash should be invalid");
...
static bool test_pawn_table_clear() {
    ...
    PawnTableEntry& probed = g_pawn_table.get_entry(entry.hash);
    ASSERT(!g_pawn_table.is_valid_entry(entry.hash, probed), "g_pawn_table_clear", "Entry should be invalid after clear()");
...
static bool test_pawn_table_collision_overwrite() {
    ...
    ASSERT(g_pawn_table.is_valid_entry(hash_a, probed_a_before), "g_pawn_table_collision_overwrite", "First entry should be valid before overwrite");

    g_pawn_table.add_entry(entry_b);

    PawnTableEntry& probed_a_after = g_pawn_table.get_entry(hash_a);
    ASSERT(!g_pawn_table.is_valid_entry(hash_a, probed_a_after), "g_pawn_table_collision_overwrite", "Old colliding entry should be invalid after overwrite");

    PawnTableEntry& probed_b_after = g_pawn_table.get_entry(hash_b);
    ASSERT(g_pawn_table.is_valid_entry(hash_b, probed_b_after), "g_pawn_table_collision_overwrite", "New colliding entry should be valid");
...
static bool test_pawn_table_replace_same_hash() {
    ...
    ASSERT(g_pawn_table.is_valid_entry(hash, probed), "g_pawn_table_replace_same_hash", "Entry should be valid after replacement");
...
static bool test_pawn_table_eval_cache(Board& b) {
    ...
    ASSERT(g_pawn_table.is_valid_entry(start_pawn_hash, start_probed) && pawn_entry_equal(start_entry, start_probed), "g_pawn_table_eval_cache", "Initial pawn entry missing or mismatched");
    ...
    ASSERT(g_pawn_table.is_valid_entry(new_pawn_hash, new_probed) && pawn_entry_equal(new_entry, new_probed), "g_pawn_table_eval_cache", "New pawn entry missing or mismatched");
