...
def _save_params(param_set, best_params, path):
...
def _run(param_set, args):
    ...
    weights_path = resolve(param_set.data_dir, param_set.prefix, '.pkl', n=args.weights)
    if not weights_path:
        print(f'No params pickle found in {param_set.data_dir}. Run the generator first.')
        sys.exit(1)
    baseline_params = load_pickle(weights_path)
    print(f'Baseline from {weights_path}\n')
    ...
    def after_trial(study, trial):
        ...
        _render(param_set.params, PATIENCE, baseline_params, best.params, trial_num, best_trial_num=best_num, score=f"{best.value:.3f}")

        if best_num != prev_best:
            prev_best = best_num
            _save_params(param_set, best.params, save_path)
    ...
    _render(param_set.params, PATIENCE, baseline_params, baseline_params, 0, score="—", first=True)
    ...
    if completed:
        best = study.best_trial
        print(f"\n\nbest trial: {best.number + 1}  |  score: {best.value:.3f}")

        for p in param_set.params:
            val = best.params[p.name]
            if p.is_int:
                val = round(val)
            print(f"  {p.name}: {val}")
