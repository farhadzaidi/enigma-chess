...
inline MovegenBenchResult run_movegen_bench(bool verbose, bool fast, bool phased) {
...
inline EngineBenchResult run_engine_bench(bool verbose, bool fast) {
...
inline BenchResults run_bench(const BenchFlags& flags) {
    ...
    if (results.ran_engine) {
        ...
        if (!results.engine.failures.empty()) {
            ...
            if (results.engine.failures.size() > static_cast<size_t>(MAX_FAILURES_TO_DISPLAY)) {
