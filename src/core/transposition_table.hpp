...
struct TTEntry {
    ...
    constexpr TTEntry() :
        hash(0), best_move(NULL_MOVE), depth(0), score(DUMMY_SCORE), node(TTNode::None), age(0) {}
...
struct TranspositionTable {
    ...
    TTEntry* get_entry(ZobristHash hash) {
        ...
        for (auto& entry : bucket) {
            if (entry.hash == hash && entry.node != TTNode::None) {
                return &entry;
            }
        }
    ...
    void add_entry(TTEntry entry) {
        ...
        for (auto& bucket_entry : bucket) {
            ...
            if (bucket_entry.hash == entry.hash || bucket_entry.node == TTNode::None) {
                bucket_entry = entry;
                return;
            }
