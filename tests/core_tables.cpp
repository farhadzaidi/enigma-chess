...
static bool test_tt_store_probe() {
    ...
    transposition_table.generation = 7;
    ...
    transposition_table.add_entry(entry);
    TTEntry* probed = transposition_table.get_entry(hash);
    if (!probed) {
...
static bool test_tt_invalid_probe() {
    ...
    if (transposition_table.get_entry(empty_hash)) {
    ...
    transposition_table.add_entry(TTEntry(stored_hash, NULL_MOVE, 5, 100, TTNode::Exact));

    if (transposition_table.get_entry(wrong_hash)) {
...
static bool test_tt_clear() {
    ...
    transposition_table.add_entry(TTEntry(hash, Move(E2, E4, MoveType::Quiet, MoveFlag::Normal), 6, 42, TTNode::Exact));
    transposition_table.clear();

    if (transposition_table.get_entry(hash)) {
        std::clog << "[FAILURE] 'tt_clear' - Entry should not exist after clear()\n";
        return false;
    }

    if (transposition_table.generation != 0) {
...
static bool test_tt_bucket_collisions_fit() {
    transposition_table.clear();
    transposition_table.generation = 4;
    ...
    for (const auto& entry : entries) {
        TTEntry* probed = transposition_table.get_entry(entry.hash);
        if (!probed) {
...
static bool test_tt_replace_same_hash() {
    ...
    transposition_table.generation = 1;
    transposition_table.add_entry(TTEntry(hash, Move(B1, C3, MoveType::Quiet, MoveFlag::Normal), 3, 50, TTNode::FailLow));

    transposition_table.generation = 5;
    transposition_table.add_entry(TTEntry(hash, Move(B1, A3, MoveType::Quiet, MoveFlag::Normal), 8, 220, TTNode::Exact));
    ...
    int matches = 0;
    for (const auto& bucket_entry : transposition_table.table[index]) {
...
static bool test_tt_bucket_replacement_by_depth() {
    transposition_table.clear();
    transposition_table.generation = 0;
    ...
    transposition_table.add_entry(TTEntry(colliding_hash(15), Move(E2, E3, MoveType::Quiet, MoveFlag::Normal), 6, 99, TTNode::FailHigh));

    if (transposition_table.get_entry(should_be_replaced)) {
        std::clog << "[FAILURE] 'tt_bucket_replacement_by_depth' - Lowest depth entry should be replaced\n";
        return false;
    }

    TTEntry* replacement = transposition_table.get_entry(colliding_hash(15));
    if (!replacement) {
...
static bool test_tt_bucket_replacement_by_age() {
    ...
    for (size_t i = 0; i < hashes.size(); i++) {
        transposition_table.generation = static_cast<int>(i);
        transposition_table.add_entry(TTEntry(
            hashes[i],
    ...
    transposition_table.generation = 8;
    ZobristHash replacement_hash = colliding_hash(25);
    transposition_table.add_entry(TTEntry(replacement_hash, Move(H2, H3, MoveType::Quiet, MoveFlag::Normal), 20, 200, TTNode::FailLow));

    if (transposition_table.get_entry(hashes[0])) {
        std::clog << "[FAILURE] 'tt_bucket_replacement_by_age' - Oldest entry should be replaced\n";
        return false;
    }

    for (size_t i = 1; i < hashes.size(); i++) {
        if (!transposition_table.get_entry(hashes[i])) {
            std::clog << "[FAILURE] 'tt_bucket_replacement_by_age' - Newer entry was replaced unexpectedly\n";
            return false;
        }
    }

    TTEntry* replacement = transposition_table.get_entry(replacement_hash);
    if (!replacement) {
...
bool test_transposition_table() {
    ...
    transposition_table.clear(); // Clean up global table state after test.
    return true;
}
#include <iostream>
...
static bool test_pawn_table_store_probe() {
    ...
    pawn_table.add_entry(entry);
    PawnTableEntry& probed = pawn_table.get_entry(entry.hash);

    if (!pawn_table.is_valid_entry(entry.hash, probed)) {
...
static bool test_pawn_table_invalid_probe() {
    ...
    PawnTableEntry& empty = pawn_table.get_entry(hash);
    if (pawn_table.is_valid_entry(hash, empty)) {
    ...
    pawn_table.add_entry(stored);

    PawnTableEntry& wrong_probed = pawn_table.get_entry(wrong_hash);
    if (pawn_table.is_valid_entry(wrong_hash, wrong_probed)) {
...
static bool test_pawn_table_collision_overwrite() {
    ...
    PawnTableEntry& slot_a = pawn_table.get_entry(hash_a);
    PawnTableEntry& slot_b = pawn_table.get_entry(hash_b);
    if (&slot_a != &slot_b) {
    ...
    pawn_table.add_entry(entry_a);
    PawnTableEntry& probed_a_before = pawn_table.get_entry(hash_a);
    if (!pawn_table.is_valid_entry(hash_a, probed_a_before)) {
        std::clog << "[FAILURE] 'pawn_table_collision_overwrite' - First entry should be valid before overwrite\n";
        return false;
    }

    pawn_table.add_entry(entry_b);

    PawnTableEntry& probed_a_after = pawn_table.get_entry(hash_a);
    if (pawn_table.is_valid_entry(hash_a, probed_a_after)) {
        std::clog << "[FAILURE] 'pawn_table_collision_overwrite' - Old colliding entry should be invalid after overwrite\n";
        return false;
    }

    PawnTableEntry& probed_b_after = pawn_table.get_entry(hash_b);
    if (!pawn_table.is_valid_entry(hash_b, probed_b_after)) {
...
static bool test_pawn_table_replace_same_hash() {
    ...
    pawn_table.add_entry(entry_1);
    pawn_table.add_entry(entry_2);

    PawnTableEntry& probed = pawn_table.get_entry(hash);
    if (!pawn_table.is_valid_entry(hash, probed)) {
...
static bool test_pawn_table_eval_cache(Board& b) {
    ...
    PawnTableEntry& start_probed = pawn_table.get_entry(start_pawn_hash);
    if (!pawn_table.is_valid_entry(start_pawn_hash, start_probed) || !pawn_entry_equal(start_entry, start_probed)) {
    ...
    PawnTableEntry& new_probed = pawn_table.get_entry(new_pawn_hash);
    if (!pawn_table.is_valid_entry(new_pawn_hash, new_probed) || !pawn_entry_equal(new_entry, new_probed)) {
...
bool test_pawn_table(Board& b) {
    ...
    pawn_table.clear(); // Clean up global table state after test.
    return true;
}
